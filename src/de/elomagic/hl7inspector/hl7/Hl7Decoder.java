...
public class Hl7Decoder {
    ...
    public Hl7Decoder(Delimiters delimiters) { d = delimiters; }
            ...
            if (c == d.escapeCharacter) {
                ...
                if (li == -1) {
                    ...
                    if (seq.length() != 0) {
                        ...
                        switch (control) {
                            case 'H': { result.append("<B>"); break; }                                  // Start higlighting
                            case 'N': { result.append("</B>"); break; }                                 // normal text (end highlighting)
                            case 'F': { result.append(d.fieldDelimiter);  break; }                      // field separator
                            case 'S': { result.append(d.componentDelimiter); break; }                   // component separator
                            case 'T': { result.append(d.subcomponentDelimiter); break; }                // subcomponent separator
                            case 'R': { result.append(d.repetitionDelimiter); break; }                  // repetition separator
                            case 'E': { result.append(d.escapeCharacter); break; }                      // escape character
                            case 'X': { result.append(((char)Integer.parseInt(value, 16))); break; }    // hexadecimal data
