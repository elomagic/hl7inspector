...
public class SimpleDialog {
    ...
    public static String ask(String text, String def) { return JOptionPane.showInputDialog(getParent(), text, def ); }

    public static void error(String text) { showExtendedDialog("Error", text, JOptionPane.ERROR_MESSAGE); }    
//    public static void error(String text) { JOptionPane.showMessageDialog(getParent(), text, "Error", JOptionPane.ERROR_MESSAGE); }
    public static void error(Exception e) { JOptionPane.showMessageDialog(getParent(), e.getMessage() + "\n\n" + getFormatedExceptionText(e, 20), "Error", JOptionPane.ERROR_MESSAGE); }
    public static void error(Exception e, String text) { JOptionPane.showMessageDialog(getParent(), text + "\n\n" + getFormatedExceptionText(e, 20), "Error", JOptionPane.ERROR_MESSAGE); }
            
    public static void info(String text) { JOptionPane.showMessageDialog(getParent(), text, "Information", JOptionPane.INFORMATION_MESSAGE); }
    public static void info(String title, String text) { showExtendedDialog(title, text, JOptionPane.INFORMATION_MESSAGE); }   
    
    public static void warn(String text) { showExtendedDialog("Warning", text, JOptionPane.WARNING_MESSAGE); }   
    public static void warn(String title, String text) { showExtendedDialog(title, text, JOptionPane.WARNING_MESSAGE); }   
//    public static void warn(String text) { JOptionPane.showMessageDialog(getParent(), text, "Warning", JOptionPane.WARNING_MESSAGE); }   
    
    public static int confirmYesNo(String text) { return JOptionPane.showConfirmDialog(getParent(), text, "Confirmation", JOptionPane.YES_NO_OPTION); }
    ...
    private static String getFormatedExceptionText(Exception e, int maxLines) {
        String s = "";
        if (e.getMessage() != null) { s = "Message: " + e.getMessage(); }
        ...
        if (lines>e.getStackTrace().length) { s = s.concat("..."); }
