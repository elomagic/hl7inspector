 ...
 * getValueAt(row, col))  they are passed to the underlying model after the
 * row numbers  have been translated via the internal mapping array. This way,
 * the TableSorter appears to hold another copy of the table  with the rows
 * in a different order.
 *
 * <p>
 * TableSorter registers itself as a listener to the underlying model,   just
 * as the JTable itself would. Events revieved from the model   are examined,
 * sometimes manipulated (typically widened), and then  passed on to the
 * TableSorter's listeners (typically the JTable).  If a change to the model
 * has invalidated the order of TableSorter's  rows, a note of this is made
 * and the sorter will resort the  rows the next time a value is requested.
 * </p>
 *
 * <p>
 ...
 * decorated with a renderer  that indicates the sorting status of each
 ...
 * the sorting status of that column through three  values: } (then back to
 * NOT_SORTED again).
 * </li>
 * <li>
